...
public class AddNewInstruments {
    ...
    static void addNewInstruments(String instrument, DaoFactory daoFactory, HttpSession session, User user) throws SQLException {
        String[] instrumentsArray = instrument.toLowerCase().split("[\\s\\.,]+");
        ...
        Object obj = session.getAttribute("Instruments");
        if (obj == null) {
            session.setAttribute("Instruments", instrumentsToAttribute);
        } else {
            List<Instrument> instrumentsFromAttribute = (List<Instrument>) obj;
            instrumentsFromAttribute.addAll(instrumentsToAttribute);
        }
